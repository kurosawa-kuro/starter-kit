...
Cloud Run / GKE は stdout の JSON を Cloud Logging に自動収集するため、
google-cloud-logging ライブラリが無くても structured logs が飛ぶ。
ライブラリがある環境では CloudLoggingHandler を使うように切替可能。

有効化:
  GCP_LOGGING_ENABLED=1                 # JSON structured logging を有効化
  GCP_LOGGING_USE_CLIENT=1              # google-cloud-logging client を利用
  GCP_PROJECT=my-project                # trace field 解決用（任意）
"""
...
class CloudLoggingJsonFormatter(logging.Formatter):
    """Cloud Logging が拾う構造化ログフォーマット (severity + message + trace)."""
...
def configure_logging(logger_name: str = "app") -> logging.Logger:
    """環境変数に応じて Cloud Logging / JSON logging を構成する。"""
...
def _extract_trace(request: Request) -> str | None:
    """X-Cloud-Trace-Context から Cloud Logging 用 trace を取り出す。"""
